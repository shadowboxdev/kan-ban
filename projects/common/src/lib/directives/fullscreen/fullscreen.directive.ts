...
@Directive({
  selector: '[kbFullScreen]',
  exportAs: 'kbFullScreen',
})
export class KbFullscreenDirective {
  ...
  private _getFullScreenElement(): Element {
    const { _document }: KbFullscreenDirective = this;
    const kbFullScreenElementMap: object = {
    ...
    for (const props of Object.keys(kbFullScreenElementMap)) {
      if (_document[props]) {
